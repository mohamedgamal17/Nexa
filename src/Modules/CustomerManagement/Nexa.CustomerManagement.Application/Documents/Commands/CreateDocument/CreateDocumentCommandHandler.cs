...
namespace Nexa.CustomerManagement.Application.Documents.Commands.CreateDocument
{
    public class CreateDocumentCommandHandler : IApplicationRequestHandler<CreateDocumentCommand, DocumentDto>
    {
        ...
        public CreateDocumentCommandHandler(ICustomerManagementRepository<Customer> customerRepository, ICustomerManagementRepository<CustomerApplication> customerApplicationRepository, ICustomerManagementRepository<Document> documentRepository, ISecurityContext securityContext, IDocumentResponseFactory documentResponseFactory, IKYCProvider kycProvider, IApplicationAuthorizationService applicationAuthorizationService)
        {
        ...
        public async Task<Result<DocumentDto>> Handle(CreateDocumentCommand request, CancellationToken cancellationToken)
        {
            ...
            {
                return new Result<DocumentDto>(new EntityNotFoundException(typeof(CustomerApplication), request.CustomerApplicationId));
            }
            ...
            {
                return new Result<DocumentDto>(authorizationRequirment.Exception!);
            }
